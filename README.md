...
> `syncPilot` push listener are in, and so is the everyday CLI surface —
> `discover`, `status`, `info`, `on`, `off`, `toggle`, `set` and `scenes`.
> `watch` and `bench` are still stubbed, and the streaming write path is still
> planned. The API will change without warning until `0.1.0`.
>
